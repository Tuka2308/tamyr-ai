...
/**
 * Ученик в списке класса вместе с происхождением записи.
 *
 * `demo` — синтетический seed-профиль дня 2, за ним не стоит реальный ребёнок.
 * `live` — настоящая регистрация через онбординг.
 *
 * Поле обязательное, а не опциональное, намеренно: смешивать синтетику
 * с реальными данными молча нельзя, и типы это требование закрепляют.
 */
export type ClassStudent = Student & { origin: "demo" | "live" };
...
export type Profile = {
  /**
   * Стабильный идентификатор ученика, crypto.randomUUID().
   * Заводится один раз при первом онбординге и переживает повторные
   * прохождения: иначе каждая пересдача создавала бы нового ученика
   * в списке учителя.
   */
  id: string;
  /** Имя. Единственное, что ученик вводит о себе; учитель видит его в списке. */
  name: string;
  grade: Grade;
  subject: Subject;
